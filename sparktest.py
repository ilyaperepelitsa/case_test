...
data_from_file_conv.map(lambda row: int(row[16])).take(10)

data_from_file_conv.filter(lambda row: row[16] != '-99').count()
data_from_file_conv.filter(lambda row: row[16] != -99).take(10)
data_from_file_conv.count()

data_from_file_conv.map(lambda row: row[5]).distinct().collect()
